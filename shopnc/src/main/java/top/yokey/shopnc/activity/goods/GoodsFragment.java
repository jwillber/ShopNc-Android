...
public class GoodsFragment extends BaseFragment {
    ...
    public void initEven() {
        ...
        addCartTextView.setOnClickListener(view -> {
            if (!haveGoods) {
                BaseSnackBar.get().show(headerRelativeLayout, "没货啦！");
                return;
            }
    ...
    private void addCart() {
        ...
        BaseSnackBar.get().showHandler(headerRelativeLayout);
        ...
        MemberCartModel.get().cartAdd(goodsId, chooseNumberEditText.getText().toString(), new BaseHttpListener() {
            ...
            public void onFailure(String reason) {
                BaseSnackBar.get().show(headerRelativeLayout, reason);
            }
        });
    ...
    private void handlerData() {
        ...
        try {
            ...
            JSONObject goodsInfoJSONObject = new JSONObject(mainJSONObject.getString("goods_info"));
            ...
            if (goodsInfoJSONObject.has("promotion_type")) {
                ...
                moneyTextView.append(goodsInfoJSONObject.getString("promotion_price"));
            ...
            for (int i = 0; i < specArrayList.length; i++) {
                if (i < 2) {
                    if (specArrayList[i] != null) {
                        ...
                        if (specArrayList[i].size() < 4) {
                            LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
                            linearLayoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
                            chooseValueRecyclerView[i].setLayoutManager(linearLayoutManager);
                        } else {
                            if (specArrayList[i].get(0).get("value").length() < 6) {
                                GridLayoutManager gridLayoutManager = new GridLayoutManager(getActivity(), 5);
                                chooseValueRecyclerView[i].setLayoutManager(gridLayoutManager);
                            } else {
            ...
            //服务信息
            if (goodsInfoJSONObject.has("contractlist")) {
                if (!goodsInfoJSONObject.getString("contractlist").equals("null")) {
    ...
    private void changeNumber() {
        ...
        if (TextUtils.isEmpty(chooseNumberEditText.getText().toString())) {
            BaseSnackBar.get().show(headerRelativeLayout, "数量不能为空！");
        ...
        if (number <= 0) {
            BaseSnackBar.get().show(headerRelativeLayout, "最低要买 1 件");
            number = 1;
        }
        ...
        if (!TextUtils.isEmpty(upperLimitString)) {
            ...
            if (number > upper) {
                number = upper;
                BaseSnackBar.get().show(headerRelativeLayout, "每人最高限购：" + number + " 件");
            }
        }
        ...
        if (!TextUtils.isEmpty(lowerLimitString)) {
            ...
            if (number < lower) {
                number = lower;
                BaseSnackBar.get().show(headerRelativeLayout, "最低要购买：" + number + " 件");
            }
        }
        ...
        if (number > storage) {
            number = storage;
            BaseSnackBar.get().show(headerRelativeLayout, "库存不足！");
        }
    ...
    private void favoritesAdd() {
        ...
        BaseSnackBar.get().show(headerRelativeLayout, "收藏中...");
    ...
    private void favoritesDel() {
        ...
        BaseSnackBar.get().show(headerRelativeLayout, "取消收藏中...");
    ...
    private void voucherFreeex(String tid) {
        ...
        MemberVoucherModel.get().voucherFreeex(tid, new BaseHttpListener() {
            @Override
            public void onSuccess(BaseBean baseBean) {
                BaseSnackBar.get().showSuccess(headerRelativeLayout);
            }
