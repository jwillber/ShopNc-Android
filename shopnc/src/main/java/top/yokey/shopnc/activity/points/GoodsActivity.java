...
public class GoodsActivity extends BaseActivity {
    ...
    public void initEven() {
        ...
        exchangeTextView.setOnClickListener(view -> {
            if (!haveGoods) {
                BaseSnackBar.get().show(mainToolbar, "没货啦！");
                return;
            }
    ...
    private void changeNumber() {
        ...
        if (TextUtils.isEmpty(chooseNumberEditText.getText().toString())) {
            BaseSnackBar.get().show(mainToolbar, "数量不能为空！");
        ...
        if (number <= 0) {
            BaseSnackBar.get().show(mainToolbar, "最低要买 1 件");
            number = 1;
        }
        ...
        if (!TextUtils.isEmpty(limit)) {
            ...
            if (number > upper) {
                number = upper;
                BaseSnackBar.get().show(mainToolbar, "每人最高限购：" + number + " 件");
            }
        }
        ...
        if (number > storage) {
            number = storage;
            BaseSnackBar.get().show(mainToolbar, "库存不足！");
        }
